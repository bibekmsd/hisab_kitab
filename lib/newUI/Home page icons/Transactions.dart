...
class _TransactionsPageState extends State<TransactionsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: Color.fromRGBO(18, 30, 46, 1),
        actions: [
          IconButton(
  ...
  Stream<QuerySnapshot> _getFilteredStream() {
    var query = FirebaseFirestore.instance.collection('bills').orderBy('PurchaseDate', descending: true);
    if (_selectedDate != null) {
      var startOfDay = DateTime(_selectedDate!.year, _selectedDate!.month, _selectedDate!.day);
      var endOfDay = startOfDay.add(Duration(days: 1));
      query = query.where('PurchaseDate', isGreaterThanOrEqualTo: startOfDay)
  ...
  Widget _buildTransactionsList(List<DocumentSnapshot> documents) {
    ...
    for (var doc in documents) {
      ...
      var total = (data['Products'] as List<dynamic>).fold<double>(
        0, (sum, product) => sum + ((product['quantity'] as num?) ?? 0) * ((product['price'] as num?) ?? 0)
      );
  ...
  Widget _buildDateGroup(String date, List<DocumentSnapshot> docs, double dailyTotal) {
    return Card(
  ...
  Widget _buildBillCard(DocumentSnapshot bill) {
    ...
    return Card(
      ...
      child: ListTile(
        title: Text('Bill #${bill.id}', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  Widget _buildBillDetailsSheet(DocumentSnapshot bill, ScrollController controller) {
    var billData = bill.data() as Map<String, dynamic>;
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          Text('Bill Details', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          SizedBox(height: 16),
          Text('Bill #${bill.id}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
