...
class _ShopInfoPageState extends State<ShopInfoPage> {
  ...
  Future<void> _fetchShopInfo() async {
    try {
      // Query the admin collection for the most recently logged-in admin
      QuerySnapshot adminSnapshot = await _db
          .collection('admin')
      ...
      if (adminSnapshot.docs.isNotEmpty) {
        setState(() {
          shopData = adminSnapshot.docs.first.data() as Map<String, dynamic>?;
        });
      } else {
        // If no admin found, use default values
        setState(() {
          shopData = {
            'shopName': 'Horses',
            'Address': 'Civil Mall',
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoCard(
                      icon: Icons.store,
                      title: shopData!['shopName'] ?? 'Horses',
                      subtitle: 'Shop Name',
                    ),
                    _buildInfoCard(
                      icon: Icons.location_on,
                      title: shopData!['Address'] ?? 'Civil Mall',
                      subtitle: 'Address',
                    ),
                    _buildInfoCard(
                      icon: Icons.phone,
                      title: shopData!['phoneNo'] ?? '9851122230',
                      subtitle: 'Phone Number',
                    ),
                    _buildInfoCard(
                      icon: Icons.confirmation_number,
                      title: shopData!['panNo'] ?? '087927493',
                      subtitle: 'PAN No.',
                    ),
                  ],
                ),
